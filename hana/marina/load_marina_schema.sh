...
#
# Usage: load_marina_schema_schema.sh [-U<HANA user>] 
#                                      [-P<HANA password>] 
#                                      [-H<HANA Host>]
#                                      [-D<HANA DB>]
#                                      [-S<HANA Schema>]
...
#test hana cloud
if [[ ${HOST} =~ .*"hanacloud.ondemand.com:443"$ ]]; then
	echo "using HANA Cloud"
	hdbsql_command="hdbsql -quiet -a -x -E 2 -m -n ${HOST} -u${USER} -p""${PASSWORD}"" "
else
	echo "using HANA on prem"
